...
public class Screen {
    ...
    // Version 2
    private EstrategiaConfiguracion estrategiaConfiguracion;

    public Screen() {
        this(CONFIG_FILE_NAME_DEFAULT);
    }

    // constructor antiguo, para que sea compatible con la version 1 del framework.
    ...
    // Constructor compatible con la version 2 del framework
    public Screen(EstrategiaConfiguracion estrategia) {
        var nombresDeClases = estrategia.cargarNombresDeClases();
        try {
            for (String nombreClase : nombresDeClases) {
                Class<?> clazz = Class.forName(nombreClase.trim());
                Accion instancia = (Accion) clazz.getDeclaredConstructor().newInstance();
